...
uint16_t AT24Cxx::readMem(uint16_t iAddr, uint8_t Buf[], uint16_t iCnt)
{
...
uint8_t AT24Cxx::writeMem(uint16_t iAddr, uint8_t iVal)
{
  uint8_t iRC=0;
  Wire.beginTransmission(devAddr);
#if ARDUINO >= 100
...
uint8_t AT24Cxx::writeMem(uint16_t iAddr, const char *pBuf, uint16_t iCnt)
{
